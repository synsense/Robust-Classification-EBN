...
class HeySnipsNetworkADS(BaseModel):
    def __init__(self,
        ...
        # - Create NetworkADS
        model_path_ads_net_ebn = os.path.join(self.base_path, "Resources/hey_snips_fast.json")
        ...
        if(os.path.exists(model_path_ads_net_ebn) and os.path.exists(model_path_ads_net_no_ebn)):
            ...
            self.net_ebn_perturbed = deepcopy(self.net_ebn)
            self.net_no_ebn_perturbed = deepcopy(self.net_no_ebn)

            # - Set neuron death parameters
            self.net_ebn_perturbed.lyrRes.t_start_suppress = 0.0
            self.net_ebn_perturbed.lyrRes.t_stop_suppress = 5.0
            self.net_ebn_perturbed.lyrRes.percentage_suppress = 0.4

            self.net_no_ebn_perturbed.lyrRes.t_start_suppress = 0.0
            self.net_no_ebn_perturbed.lyrRes.t_stop_suppress = 5.0
            self.net_no_ebn_perturbed.lyrRes.percentage_suppress = 0.4
        ...
        for batch_id, [batch, test_logger] in enumerate(data_loader.test_set()):
            ...
            for idx in range(len(batch)):
                ...
                if(self.verbose > 1):
                    ...
                    self.net_ebn_perturbed.lyrRes.ts_target = TSContinuous(time_base, batched_rate_net_dynamics[idx])
                    self.net_no_ebn_perturbed.lyrRes.ts_target = TSContinuous(time_base, batched_rate_net_dynamics[idx])
                ...
                test_sim_ebn_perturbed = self.net_ebn_perturbed.evolve(ts_input=ts_spiking_in, verbose=(self.verbose > 1)); self.net_ebn_perturbed.reset_all()
                test_sim_no_ebn_perturbed = self.net_no_ebn_perturbed.evolve(ts_input=ts_spiking_in, verbose=(self.verbose > 1)); self.net_no_ebn_perturbed.reset_all()                

                # - Get the output
                out_test_ebn = test_sim_ebn["output_layer_0"].samples
                out_test_no_ebn = test_sim_no_ebn["output_layer_0"].samples
                out_test_ebn_perturbed = test_sim_ebn_perturbed["output_layer_0"].samples
                out_test_no_ebn_perturbed = test_sim_no_ebn_perturbed["output_layer_0"].samples

                if(self.verbose > 1):
                    self.net_ebn.lyrRes.ts_target = None
                    self.net_no_ebn.lyrRes.ts_target = None
                    self.net_ebn_perturbed.lyrRes.ts_target = None
                    self.net_no_ebn_perturbed.lyrRes.ts_target = None

                # - Compute the final output
                final_out_ebn = out_test_ebn @ self.w_out
                final_out_no_ebn = out_test_no_ebn @ self.w_out
                final_out_ebn_perturbed = out_test_ebn_perturbed @ self.w_out
                final_out_no_ebn_perturbed = out_test_no_ebn_perturbed @ self.w_out
                ...
                error_no_ebn = np.sum(np.var(batched_rate_net_dynamics[idx]-out_test_no_ebn, axis=0, ddof=1)) / (np.sum(np.var(batched_rate_net_dynamics[idx], axis=0, ddof=1)))
                error_ebn_perturbed = np.sum(np.var(batched_rate_net_dynamics[idx]-out_test_ebn_perturbed, axis=0, ddof=1)) / (np.sum(np.var(batched_rate_net_dynamics[idx], axis=0, ddof=1)))
                error_no_ebn_perturbed = np.sum(np.var(batched_rate_net_dynamics[idx]-out_test_no_ebn_perturbed, axis=0, ddof=1)) / (np.sum(np.var(batched_rate_net_dynamics[idx], axis=0, ddof=1)))
                ...
                error_ebn_perturbed = np.mean(np.linalg.norm(batched_rate_net_dynamics[idx]-out_test_ebn_perturbed, axis=0))
                error_no_ebn_perturbed = np.mean(np.linalg.norm(batched_rate_net_dynamics[idx]-out_test_no_ebn_perturbed, axis=0))
                ...
                if(self.verbose > 0):
                    ...
                    plt.plot(time_base, final_out_ebn_perturbed, label="Spiking ebn_perturbed")
                    plt.plot(time_base, final_out_no_ebn_perturbed, label="Spiking no_ebn_perturbed")
                ...
                predicted_label_ebn_perturbed = self.get_prediction(final_out_ebn_perturbed, self.net_ebn_perturbed, self.bb_ebn, self.t0_ebn)
                predicted_label_no_ebn_perturbed = self.get_prediction(final_out_no_ebn_perturbed, self.net_no_ebn_perturbed, self.bb_no_ebn, self.t0_no_ebn)

...
class HeySnipsNetworkADS(BaseModel):
    def __init__(self,
        ...
        # - Create NetworkADS
        if(self.use_fast):
            model_path_ads_net = os.path.join(self.base_path,"Resources/hey_snips_fast.json")
        else:
            model_path_ads_net = os.path.join(self.base_path,"Resources/hey_snips_no_fast.json")

        if(os.path.exists(model_path_ads_net)):
            print("Network already trained. If you would like to retrain, please rename/delete the network...")
            sys.exit(0) # - Uncomment this to continue training a given model
    ...
    def check_balance(self, ts_spiking_in):
        ...
        ts_out_val_no_fast = val_sim_no_fast["output_layer_0"]
        ...
        f_out = ts_out_val.samples
        nf_out = ts_out_val_no_fast.samples
        target = 0.0005*ts_spiking_in.samples

        error_fast = np.mean(np.linalg.norm(f_out-target,axis=0))
        error_no_fast = np.mean(np.linalg.norm(nf_out-target,axis=0))
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_id, [batch, train_logger] in enumerate(data_loader.train_set()):
                ...
                for idx in range(len(batch)):
                    ...
                    # - Check if the fast connections implement balance
                    if(self.use_fast and idx==0 and batch_id==0 and epoch==0):
                    ...
                    print("Target label", target_labels[idx], "Predicted label", predicted_label, ("Avg. training acc. %.4f" % (training_acc)), ("Avg. reconstruction error %.4f" % (reconstruction_acc)), "K", self.net.lyrRes.k, "Err.:", error, flush=True)
                    print("--------------------", flush=True)
    ...
    def test(self, data_loader, fn_metrics):
        ...
        # - Save the network
        if(self.use_fast):
            param_string = "Resources/hey_snips_fast.json"
        else:
            param_string = "Resources/hey_snips_no_fast.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use-fast', default=False, action='store_true', help="Utilize the EBN connections.")
